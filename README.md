...
> Twenty of thirty planned Sigma detections are implemented. Microsoft Sentinel is the only supported target, with nineteen current rules rendering as deterministic, disabled Scheduled rules.
...
customer value is stored in this repository. Three bounded implementation waves
...
The logical manifest, compact package contract, pinned pySigma boundary, and bounded local fixture evaluator are implemented and locally verified. The catalogue now contains twenty authored Sigma detections. Wave 3 adds Windows recovery inhibition, event-log clearing, PowerShell Defender weakening, Entra federation-trust changes and certutil remote transfer. Together the catalogue has sixty positive and eighty negative synthetic cases.

The Sentinel preview pins the Kusto backend and binds nineteen detections across `SigninLogs`, `AuditLogs`, `DeviceProcessEvents` and `AADUserRiskEvents`; all nineteen generated KQL queries match committed Golden snapshots. Separate authorized read-only target probes accepted every generated predicate in bounded aggregate form. A positive or negative live result is only query-acceptance evidence: no raw telemetry, result count, or target identifier is stored, and no result is a deployment or production-readiness claim.
...
The nineteen preview-bound detections also render into complete Scheduled-rule REST
...
view: fifteen unique ATT&CK techniques across nine tactics, five logical data
...
The final operations contract derives expected rule executions from the nineteen
