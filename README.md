...
> Fifteen of thirty planned Sigma detections are implemented. Microsoft Sentinel is the only supported target, with fourteen current rules rendering as deterministic, disabled Scheduled rules.
...
customer value is stored in this repository. Two bounded implementation waves
...
The logical manifest, compact package contract, pinned pySigma boundary, and bounded local fixture evaluator are implemented and locally verified. The catalogue now contains fifteen authored Sigma detections. Wave 2 adds potential LSASS dumping through comsvcs, remote-content execution through mshta and regsvr32, Conditional Access policy deletion, and owner addition to an Entra application or service principal. Together the catalogue has forty-five positive and sixty negative synthetic cases.

The Sentinel preview pins the Kusto backend and binds fourteen detections across `SigninLogs`, `AuditLogs`, `DeviceProcessEvents` and `AADUserRiskEvents`; all fourteen generated KQL queries match committed Golden snapshots. Separate authorized read-only target probes accepted every generated predicate in bounded aggregate form. A positive or negative live result is only query-acceptance evidence: no raw telemetry, result count, or target identifier is stored, and no result is a deployment or production-readiness claim.
...
The fourteen preview-bound detections also render into complete Scheduled-rule REST
...
view: ten unique ATT&CK techniques across six tactics, five logical data
...
The final operations contract derives expected rule executions from the fourteen
