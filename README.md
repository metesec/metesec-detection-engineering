...
> Twenty-five of thirty planned Sigma detections are implemented. Microsoft Sentinel is the only supported target, with twenty-four current rules rendering as deterministic, disabled Scheduled rules.
...
customer value is stored in this repository. Four bounded implementation waves
...
The logical manifest, compact package contract, pinned pySigma boundary, and bounded local fixture evaluator are implemented and locally verified. The catalogue now contains twenty-five authored Sigma detections. Wave 4 adds an all-users privileged delegated-permission grant, Entra strong-authentication disablement, SAM or SECURITY hive export through `reg.exe`, remote process creation through WMI or CIM and BITSAdmin remote transfer. Together the catalogue has seventy-five positive and one hundred negative synthetic cases.

The Sentinel preview pins the Kusto backend and binds twenty-four detections across `SigninLogs`, `AuditLogs`, `DeviceProcessEvents` and `AADUserRiskEvents`; all twenty-four generated KQL queries match committed Golden snapshots. Separate authorized read-only target probes accepted every generated predicate in bounded aggregate form. A positive or negative live result is only query-acceptance evidence: no raw telemetry, result count, or target identifier is stored, and no result is a deployment or production-readiness claim.
...
The twenty-four preview-bound detections also render into complete Scheduled-rule REST
...
view: nineteen unique ATT&CK techniques across ten tactics, five logical data
...
The final operations contract derives expected rule executions from the twenty-four
