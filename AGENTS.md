...
- Expansion baseline: twenty-five of thirty Sigma detections are implemented; Waves 1 through 4 are complete and the next bounded milestone must review Wave 5 separately rather than bulk-importing community rules
...
- Catalogue: twenty-five experimental packages. `MSEC-DET-0001` through `MSEC-DET-0005` retain the original Windows service, Entra sign-in and Entra application-change coverage; Wave 1 adds `MSEC-DET-0006` through `MSEC-DET-0010`; Wave 2 adds `MSEC-DET-0011` through `MSEC-DET-0015`; Wave 3 adds `MSEC-DET-0016` through `MSEC-DET-0020`; Wave 4 adds `MSEC-DET-0021` highly privileged delegated permission for all users, `MSEC-DET-0022` Entra strong-authentication disablement, `MSEC-DET-0023` SAM or SECURITY hive export through `reg.exe`, `MSEC-DET-0024` remote process creation through WMI or CIM and `MSEC-DET-0025` BITSAdmin remote transfer
- Generated discovery catalogue: deterministic `catalog/index.json` and `CATALOGUE.md` are derived from the twenty-five manifests, implementation-local fixture indexes, and explicit Sentinel preview profile; neither output contains timestamps, environment identifiers, or live result data
- Catalogue contract and validation: JSON Schema version 1, three generator tests, and a stale-output gate are included in the aggregate repository check; the current output reports twenty-five implementations, seventy-five positive cases, one hundred negative cases, and twenty-four Sentinel preview bindings
...
- Local `v0.4.0` candidate: the deterministic release build contains 313 members, including 312 allowlisted sources, all twenty-five Sigma packages, 175 synthetic fixtures, twenty-four Golden KQL queries and the read-only Sentinel inventory guide; its current SHA-256 is `940cca19a686b51b42206afcd59dcce53a89d3abe7acfab38a6b8f9bce9023c0`, while the published `v0.1.0` remains unchanged
- Portable implementations: twenty-five structurally valid Sigma rules, one per package
- Synthetic evidence: seventy-five positive and one hundred negative flat event fixtures, all explicitly marked synthetic and all passing locally
...
- Sigma structural validation: exact-version gate, two-sided in-memory parser self-test, and automatic Package v1 `rule.yml` discovery validate twenty-five sources containing twenty-five rules
...
- Evaluator tests: six passing unit cases plus 175 passing committed fixture expectations
...
- Sentinel preview scope: twenty-four rules are explicitly bound across `SigninLogs`, `AuditLogs`, `DeviceProcessEvents` and `AADUserRiskEvents`; all twenty-four complete analyst-facing KQL queries match committed Golden snapshots. Audit bindings project initiators as supported entities and keep variable target-resource data neutral unless the resource type is guaranteed; endpoint bindings project only the observed account name
- Sentinel analytics-rule profile: version 1 JSON Schema and executable loader bind exactly the same twenty-four detections to explicit schedule, threshold, suppression, event-grouping and incident settings; missing, additional, duplicated, reordered, active or malformed entries fail closed
- Sentinel analytics-rule renderer: all twenty-four bindings produce deterministic Microsoft SecurityInsights API `2025-09-01` Scheduled-rule request bodies plus separate provenance manifests; logical metadata comes from `manifest.json`, KQL and entity mappings come from the reviewed compiler output contract, stable rule UUIDs derive from the immutable detection ID, and every rendered rule is disabled. Current ATT&CK `Defense Impairment` and `Stealth` source mappings remain exact in provenance but are omitted from the older target tactic enum rather than mislabeled as another tactic
...
- Generated coverage report: deterministic `coverage/index.json` and `COVERAGE.md` derive only from logical manifests, the Sentinel preview and the Sentinel data-source contract; the report records thirty-one ATT&CK mappings across nineteen techniques and ten tactics, five logical data sources, four Sentinel source contracts and the one intentionally unbound detection without a percentage score or live state
...
- Live target probes: authorized read-only workspace checks accepted all twenty-four generated predicates in bounded aggregate form. In Wave 1, `MSEC-DET-0006` and `MSEC-DET-0008` returned no match while `MSEC-DET-0007`, `MSEC-DET-0009` and `MSEC-DET-0010` returned a positive result. In Wave 2, `MSEC-DET-0011` through `MSEC-DET-0013` returned no match while `MSEC-DET-0014` and `MSEC-DET-0015` returned a positive result. All five Wave 3 predicates returned no match. In Wave 4, `MSEC-DET-0021` returned a positive result while `MSEC-DET-0022` through `MSEC-DET-0025` returned no match in the current 30-day aggregate baseline; no raw row, exact count, user, device, tenant, subscription or workspace identifier is stored in the repository
...
- Current local validation: the complete aggregate repository check passes with 83 unit tests plus all structural, generated-output and Golden gates; all 175 synthetic fixture expectations pass, twenty-four disabled Sentinel rule bodies render successfully and the deterministic `v0.4.0` release candidate builds successfully
...
Review and implement Sigma expansion Wave 5 to reach 30 of 30. Each rule must
