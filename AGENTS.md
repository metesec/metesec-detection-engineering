...
- Expansion baseline: fifteen of thirty Sigma detections are implemented; Waves 1 and 2 are complete and the next bounded milestone must review Wave 3 separately rather than bulk-importing community rules
...
- Catalogue: fifteen experimental packages. `MSEC-DET-0001` through `MSEC-DET-0005` retain the original Windows service, Entra sign-in and Entra application-change coverage; Wave 1 adds `MSEC-DET-0006` through `MSEC-DET-0010`; Wave 2 adds `MSEC-DET-0011` potential comsvcs LSASS dumping, `MSEC-DET-0012` remote mshta execution, `MSEC-DET-0013` remote regsvr32 scriptlet or DLL execution, `MSEC-DET-0014` Conditional Access policy deletion and `MSEC-DET-0015` application or service-principal owner addition
- Generated discovery catalogue: deterministic `catalog/index.json` and `CATALOGUE.md` are derived from the fifteen manifests, implementation-local fixture indexes, and explicit Sentinel preview profile; neither output contains timestamps, environment identifiers, or live result data
- Catalogue contract and validation: JSON Schema version 1, three generator tests, and a stale-output gate are included in the aggregate repository check; the current output reports fifteen implementations, forty-five positive cases, sixty negative cases, and fourteen Sentinel preview bindings
...
- Local `v0.4.0` candidate: the deterministic release build contains 203 members, including 202 allowlisted sources, all fifteen Sigma packages, 105 synthetic fixtures, fourteen Golden KQL queries and the read-only Sentinel inventory guide; its current SHA-256 is `c6bd2ea320d4fb987cfe4a80d6a48cf008c94300e7d7840263333a974c248873`, while the published `v0.1.0` remains unchanged
- Portable implementations: fifteen structurally valid Sigma rules, one per package
...
- Sigma structural validation: exact-version gate, two-sided in-memory parser self-test, and automatic Package v1 `rule.yml` discovery validate fifteen sources containing fifteen rules
...
- Evaluator tests: six passing unit cases plus 105 passing committed fixture expectations
...
- Sentinel preview scope: fourteen rules are explicitly bound across `SigninLogs`, `AuditLogs`, `DeviceProcessEvents` and `AADUserRiskEvents`; all fourteen complete analyst-facing KQL queries match committed Golden snapshots. The owner-addition binding projects the first target as neutral resource type, ID and name fields and deliberately does not map it as a CloudApplication, because live `AuditLogs` validation showed that Entra may return a User or ServicePrincipal in that array position
- Sentinel analytics-rule profile: version 1 JSON Schema and executable loader bind exactly the same fourteen detections to explicit schedule, threshold, suppression, event-grouping and incident settings; missing, additional, duplicated, reordered, active or malformed entries fail closed
- Sentinel analytics-rule renderer: all fourteen bindings produce deterministic Microsoft SecurityInsights API `2025-09-01` Scheduled-rule request bodies plus separate provenance manifests; logical metadata comes from `manifest.json`, KQL and entity mappings come from the reviewed compiler output contract, stable rule UUIDs derive from the immutable detection ID, and every rendered rule is disabled
...
- Generated coverage report: deterministic `coverage/index.json` and `COVERAGE.md` derive only from logical manifests, the Sentinel preview and the Sentinel data-source contract; the report records fifteen ATT&CK mappings across ten techniques and six tactics, five logical data sources, four Sentinel source contracts and the one intentionally unbound detection without a percentage score or live state
...
- Live target probes: authorized read-only workspace checks accepted all fourteen generated predicates in bounded aggregate form. In Wave 1, `MSEC-DET-0006` and `MSEC-DET-0008` returned no match while `MSEC-DET-0007`, `MSEC-DET-0009` and `MSEC-DET-0010` returned a positive result. In Wave 2, `MSEC-DET-0011` through `MSEC-DET-0013` returned no match while `MSEC-DET-0014` and `MSEC-DET-0015` returned a positive result; no raw row, exact count, user, device, tenant, subscription or workspace identifier is stored in the repository
...
- Current local validation: the complete aggregate repository check passes with 81 unit tests plus all structural, generated-output and Golden gates; all 105 synthetic fixture expectations pass, fourteen disabled Sentinel rule bodies render successfully and the deterministic `v0.4.0` release candidate builds successfully
...
Review and implement Sigma expansion Wave 3 to reach 20 of 30. Each rule must
