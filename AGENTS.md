...
- Expansion baseline: twenty of thirty Sigma detections are implemented; Waves 1 through 3 are complete and the next bounded milestone must review Wave 4 separately rather than bulk-importing community rules
...
- Catalogue: twenty experimental packages. `MSEC-DET-0001` through `MSEC-DET-0005` retain the original Windows service, Entra sign-in and Entra application-change coverage; Wave 1 adds `MSEC-DET-0006` through `MSEC-DET-0010`; Wave 2 adds `MSEC-DET-0011` through `MSEC-DET-0015`; Wave 3 adds `MSEC-DET-0016` native Windows recovery inhibition, `MSEC-DET-0017` Windows event-log clearing, `MSEC-DET-0018` PowerShell Defender weakening, `MSEC-DET-0019` Entra federation-trust changes and `MSEC-DET-0020` certutil remote transfer
- Generated discovery catalogue: deterministic `catalog/index.json` and `CATALOGUE.md` are derived from the twenty manifests, implementation-local fixture indexes, and explicit Sentinel preview profile; neither output contains timestamps, environment identifiers, or live result data
- Catalogue contract and validation: JSON Schema version 1, three generator tests, and a stale-output gate are included in the aggregate repository check; the current output reports twenty implementations, sixty positive cases, eighty negative cases, and nineteen Sentinel preview bindings
...
- Local `v0.4.0` candidate: the deterministic release build contains 258 members, including 257 allowlisted sources, all twenty Sigma packages, 140 synthetic fixtures, nineteen Golden KQL queries and the read-only Sentinel inventory guide; its current SHA-256 is `95d714202e15fc98e9fa91a32f914c1767e10cf88fa231da36564f5c51380974`, while the published `v0.1.0` remains unchanged
- Portable implementations: twenty structurally valid Sigma rules, one per package
- Synthetic evidence: forty-five positive and sixty negative flat event fixtures, all explicitly marked synthetic and all passing locally
...
- Sigma structural validation: exact-version gate, two-sided in-memory parser self-test, and automatic Package v1 `rule.yml` discovery validate twenty sources containing twenty rules
...
- Evaluator tests: six passing unit cases plus 140 passing committed fixture expectations
...
- Sentinel preview scope: nineteen rules are explicitly bound across `SigninLogs`, `AuditLogs`, `DeviceProcessEvents` and `AADUserRiskEvents`; all nineteen complete analyst-facing KQL queries match committed Golden snapshots. The owner-addition and federation-change bindings project the first target as neutral resource type, ID and name fields and deliberately do not map it as a CloudApplication where the resource type is not guaranteed
- Sentinel analytics-rule profile: version 1 JSON Schema and executable loader bind exactly the same nineteen detections to explicit schedule, threshold, suppression, event-grouping and incident settings; missing, additional, duplicated, reordered, active or malformed entries fail closed
- Sentinel analytics-rule renderer: all nineteen bindings produce deterministic Microsoft SecurityInsights API `2025-09-01` Scheduled-rule request bodies plus separate provenance manifests; logical metadata comes from `manifest.json`, KQL and entity mappings come from the reviewed compiler output contract, stable rule UUIDs derive from the immutable detection ID, and every rendered rule is disabled. Current ATT&CK `Defense Impairment` source mappings remain exact in provenance but are omitted from the older target tactic enum rather than mislabeled as `DefenseEvasion`
...
- Generated coverage report: deterministic `coverage/index.json` and `COVERAGE.md` derive only from logical manifests, the Sentinel preview and the Sentinel data-source contract; the report records twenty-one ATT&CK mappings across fifteen techniques and nine tactics, five logical data sources, four Sentinel source contracts and the one intentionally unbound detection without a percentage score or live state
...
- Live target probes: authorized read-only workspace checks accepted all nineteen generated predicates in bounded aggregate form. In Wave 1, `MSEC-DET-0006` and `MSEC-DET-0008` returned no match while `MSEC-DET-0007`, `MSEC-DET-0009` and `MSEC-DET-0010` returned a positive result. In Wave 2, `MSEC-DET-0011` through `MSEC-DET-0013` returned no match while `MSEC-DET-0014` and `MSEC-DET-0015` returned a positive result. All five Wave 3 predicates returned no match in the current 30-day aggregate baseline; no raw row, exact count, user, device, tenant, subscription or workspace identifier is stored in the repository
...
- Current local validation: the complete aggregate repository check passes with 83 unit tests plus all structural, generated-output and Golden gates; all 140 synthetic fixture expectations pass, nineteen disabled Sentinel rule bodies render successfully and the deterministic `v0.4.0` release candidate builds successfully
...
Review and implement Sigma expansion Wave 4 to reach 25 of 30. Each rule must
use observed fields, synthetic positive and negative fixtures, an explicit
Sentinel binding, reviewed generated KQL and a disabled analytics-rule body.
Do not import bulk community content, store raw live query output or add native
KQL. Keep public pull-request execution disabled until the runner gains hard
per-job isolation.
