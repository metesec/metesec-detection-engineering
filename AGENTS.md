...
- Catalogue: three experimental packages: `MSEC-DET-0001` for Windows service installation from selected public-user or temporary paths, `MSEC-DET-0002` for successful Microsoft Entra sign-ins from selected legacy client categories, and `MSEC-DET-0003` for successful Microsoft Entra sign-ins assessed as high risk during sign-in
- Portable implementations: three structurally valid Sigma rules, one per package
- Synthetic evidence: nine positive and twelve negative flat event fixtures, all explicitly marked synthetic and all passing locally
...
- Sigma structural validation: exact-version gate, two-sided in-memory parser self-test, and automatic Package v1 `rule.yml` discovery validate three sources containing three rules
...
- Evaluator tests: six passing unit cases plus twenty-one passing committed fixture expectations
...
- Sentinel preview scope: `MSEC-DET-0002` and `MSEC-DET-0003` are explicitly bound to the public `SigninLogs` table contract; both generated KQL queries match committed Golden snapshots
- Live target probes: authorized read-only workspace checks confirmed populated source fields and accepted both exact generated predicates; `MSEC-DET-0002` produced a valid negative result and `MSEC-DET-0003` a valid positive result, while no raw row, aggregate count, user, device, tenant, subscription, or workspace identifier was stored in the repository
...
Add the fourth reviewed detection using the same package and evidence boundary, choosing a precise signal supported by available telemetry rather than a high-volume aggregate event. Extend the Sentinel preview only when a rule has a reviewed explicit table binding, Golden query, and separately recorded target validation; do not deploy from the preview.
