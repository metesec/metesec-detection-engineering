...
- Expansion baseline: all thirty planned Sigma detections are implemented; Waves 1 through 5 are complete and the remaining `0.4` work is release-readiness review rather than further rule expansion
...
- Catalogue: thirty experimental packages. `MSEC-DET-0001` through `MSEC-DET-0005` retain the original Windows service, Entra sign-in and Entra application-change coverage; Wave 1 adds `MSEC-DET-0006` through `MSEC-DET-0010`; Wave 2 adds `MSEC-DET-0011` through `MSEC-DET-0015`; Wave 3 adds `MSEC-DET-0016` through `MSEC-DET-0020`; Wave 4 adds `MSEC-DET-0021` through `MSEC-DET-0025`; Wave 5 adds `MSEC-DET-0026` successful Entra ROPC sign-in, `MSEC-DET-0027` Rundll32 inline script through MSHTML, `MSEC-DET-0028` remote scheduled-task creation, `MSEC-DET-0029` local Administrators-group membership addition and `MSEC-DET-0030` NTDSutil IFM creation
- Generated discovery catalogue: deterministic `catalog/index.json` and `CATALOGUE.md` are derived from the thirty manifests, implementation-local fixture indexes, and explicit Sentinel preview profile; neither output contains timestamps, environment identifiers, or live result data
- Catalogue contract and validation: JSON Schema version 1, three generator tests, and a stale-output gate are included in the aggregate repository check; the current output reports thirty implementations, ninety positive cases, one hundred twenty negative cases, and twenty-nine Sentinel preview bindings
...
- Local `v0.4.0` candidate: the deterministic release build contains 368 members, including 367 allowlisted sources, all thirty Sigma packages, 210 synthetic fixtures, twenty-nine Golden KQL queries and the read-only Sentinel inventory guide; its current SHA-256 is `2bb28e93cd1a039d8f2c984997dbb57697fcd19f627b313a53a41cec3dd598c1`, while the published `v0.1.0` remains unchanged
- Portable implementations: thirty structurally valid Sigma rules, one per package
- Synthetic evidence: ninety positive and one hundred twenty negative flat event fixtures, all explicitly marked synthetic and all passing locally
...
- Sigma structural validation: exact-version gate, two-sided in-memory parser self-test, and automatic Package v1 `rule.yml` discovery validate thirty sources containing thirty rules
...
- Evaluator tests: six passing unit cases plus 210 passing committed fixture expectations
...
- Sentinel preview scope: twenty-nine rules are explicitly bound across `SigninLogs`, `AuditLogs`, `DeviceProcessEvents` and `AADUserRiskEvents`; all twenty-nine complete analyst-facing KQL queries match committed Golden snapshots. Audit bindings project initiators as supported entities and keep variable target-resource data neutral unless the resource type is guaranteed; sign-in bindings project normalized Account, IP and CloudApplication context; endpoint bindings project only the observed account name
- Sentinel analytics-rule profile: version 1 JSON Schema and executable loader bind exactly the same twenty-nine detections to explicit schedule, threshold, suppression, event-grouping and incident settings; missing, additional, duplicated, reordered, active or malformed entries fail closed
- Sentinel analytics-rule renderer: all twenty-nine bindings produce deterministic Microsoft SecurityInsights API `2025-09-01` Scheduled-rule request bodies plus separate provenance manifests; logical metadata comes from `manifest.json`, KQL and entity mappings come from the reviewed compiler output contract, stable rule UUIDs derive from the immutable detection ID, and every rendered rule is disabled. Current ATT&CK `Defense Impairment` and `Stealth` source mappings remain exact in provenance but are omitted from the older target tactic enum rather than mislabeled as another tactic
- Sentinel entity output: sign-in rules return normalized Account, IP and CloudApplication fields; audit rules return initiating entities and explicitly typed neutral target fields where a target entity would be ambiguous; endpoint rules map only the observed account name; the user-risk rule maps the Entra account. No Windows SID is mislabeled as an Entra object ID
- Sentinel data-source contract: version 1 covers `SigninLogs`, `AuditLogs`, `DeviceProcessEvents` and `AADUserRiskEvents` with stable source IDs, exact preview consumers, event-time columns, required Kusto fields and types, and explicit six-hour degraded and one-day unavailable reference thresholds
...
- Generated coverage report: deterministic `coverage/index.json` and `COVERAGE.md` derive only from logical manifests, the Sentinel preview and the Sentinel data-source contract; the report records thirty-nine ATT&CK mappings across twenty-three techniques and ten tactics, five logical data sources, four Sentinel source contracts and the one intentionally unbound detection without a percentage score or live state
...
- Live target probes: authorized read-only workspace checks accepted all twenty-nine generated predicates in bounded aggregate form. In Wave 1, `MSEC-DET-0006` and `MSEC-DET-0008` returned no match while `MSEC-DET-0007`, `MSEC-DET-0009` and `MSEC-DET-0010` returned a positive result. In Wave 2, `MSEC-DET-0011` through `MSEC-DET-0013` returned no match while `MSEC-DET-0014` and `MSEC-DET-0015` returned a positive result. All five Wave 3 predicates returned no match. In Wave 4, `MSEC-DET-0021` returned a positive result while `MSEC-DET-0022` through `MSEC-DET-0025` returned no match. In Wave 5, `MSEC-DET-0026` returned a positive result while `MSEC-DET-0027` through `MSEC-DET-0030` returned no match in the current 30-day aggregate baseline; a remote MSI-installation candidate was rejected because its baseline was too active for a portable default alert. No raw row, exact count, user, device, tenant, subscription or workspace identifier is stored in the repository
...
- Current local validation: the complete aggregate repository check passes with 83 unit tests plus all structural, generated-output and Golden gates; all 210 synthetic fixture expectations pass, twenty-nine disabled Sentinel rule bodies render successfully and the deterministic `v0.4.0` release candidate builds successfully
...
Perform the `v1.0.0` release-readiness review over the complete thirty-rule
Detection Pack: verify versioning and release notes, reproduce the public
archive and checksum from clean checkouts, review protected-main publication
steps and confirm the Forgejo validation context before tagging. Do not add
native KQL, deploy or enable Sentinel rules, publish a separate target archive,
store raw live query output or expand the rule count during this milestone.
