...
- Expansion baseline: thirty-five of fifty planned Sigma detections are implemented; Waves 1 through 6 are complete and Wave 7 is the next bounded expansion milestone
...
- Catalogue: thirty-five experimental packages. `MSEC-DET-0001` through `MSEC-DET-0005` retain the original Windows service, Entra sign-in and Entra application-change coverage; Wave 1 adds `MSEC-DET-0006` through `MSEC-DET-0010`; Wave 2 adds `MSEC-DET-0011` through `MSEC-DET-0015`; Wave 3 adds `MSEC-DET-0016` through `MSEC-DET-0020`; Wave 4 adds `MSEC-DET-0021` through `MSEC-DET-0025`; Wave 5 adds `MSEC-DET-0026` through `MSEC-DET-0030`; Wave 6 adds `MSEC-DET-0031` suspicious web-server child process, `MSEC-DET-0032` audit-policy clearing, `MSEC-DET-0033` Windows Firewall profile disabling, `MSEC-DET-0034` remote service creation through `sc.exe` and `MSEC-DET-0035` Certutil decoding
- Generated discovery catalogue: deterministic `catalog/index.json` and `CATALOGUE.md` are derived from the thirty-five manifests, implementation-local fixture indexes, and explicit Sentinel preview profile; neither output contains timestamps, environment identifiers, or live result data
- Catalogue contract and validation: JSON Schema version 1, three generator tests, and a stale-output gate are included in the aggregate repository check; the current output reports thirty-five implementations, one hundred five positive cases, one hundred forty negative cases, and thirty-four Sentinel preview bindings
...
- Local `v0.4.0` candidate: the deterministic release build contains 423 members, including 422 allowlisted sources, all thirty-five Sigma packages, 245 synthetic fixtures, thirty-four Golden KQL queries and the read-only Sentinel inventory guide; its current SHA-256 is `11c1e14fa469207878af684c572f3bf51026ec8e68b00989e525d6faf285ef44`, while the published `v0.1.0` remains unchanged
- Portable implementations: thirty-five structurally valid Sigma rules, one per package
- Synthetic evidence: one hundred five positive and one hundred forty negative flat event fixtures, all explicitly marked synthetic and all passing locally
...
- Sigma structural validation: exact-version gate, two-sided in-memory parser self-test, and automatic Package v1 `rule.yml` discovery validate thirty-five sources containing thirty-five rules
...
- Evaluator tests: six passing unit cases plus 245 passing committed fixture expectations
...
- Generated coverage report: deterministic `coverage/index.json` and `COVERAGE.md` derive only from logical manifests, the Sentinel preview and the Sentinel data-source contract; the report records forty-five ATT&CK mappings across twenty-seven techniques and ten tactics, five logical data sources, four Sentinel source contracts and the one intentionally unbound detection without a percentage score or live state
...
- Live target probes: authorized read-only workspace checks accepted all thirty-four generated predicates in bounded aggregate form. In Wave 1, `MSEC-DET-0006` and `MSEC-DET-0008` returned no match while `MSEC-DET-0007`, `MSEC-DET-0009` and `MSEC-DET-0010` returned a positive result. In Wave 2, `MSEC-DET-0011` through `MSEC-DET-0013` returned no match while `MSEC-DET-0014` and `MSEC-DET-0015` returned a positive result. All five Wave 3 predicates returned no match. In Wave 4, `MSEC-DET-0021` returned a positive result while `MSEC-DET-0022` through `MSEC-DET-0025` returned no match. In Wave 5, `MSEC-DET-0026` returned a positive result while `MSEC-DET-0027` through `MSEC-DET-0030` returned no match. All five Wave 6 predicates returned no match in the current 30-day aggregate baseline. A remote MSI-installation candidate was rejected because its baseline was too active for a portable default alert; a local-account-creation candidate was deferred because process telemetry proves an attempted command rather than object creation. No raw row, exact count, user, device, tenant, subscription or workspace identifier is stored in the repository
...
- Current local validation: the complete aggregate repository check passes with 83 unit tests plus all structural, generated-output and Golden gates; all 245 synthetic fixture expectations pass, thirty-four disabled Sentinel rule bodies render successfully and two consecutive deterministic `v0.4.0` release builds produced the same archive size and SHA-256
...
Research and review Wave 7 toward 40 of 50 Sigma detections. Select five
non-duplicate candidates from confirmed target telemetry, validate the required
fields and exact predicates read-only, and add only rules that remain faithful
and useful as portable Sigma. Do not add native KQL, deploy or enable Sentinel
rules, publish a separate target archive or store raw live query output. Release
readiness begins only after the fifty-rule pack is complete. Keep public
pull-request execution disabled until the runner gains hard per-job isolation.
