...
- Sentinel preview scope: fourteen rules are explicitly bound across `SigninLogs`, `AuditLogs`, `DeviceProcessEvents` and `AADUserRiskEvents`; all fourteen complete analyst-facing KQL queries match committed Golden snapshots
...
- Sentinel entity output: sign-in rules return normalized Account, IP and CloudApplication fields; audit rules return initiating entities and a semantically bounded target mapping; endpoint rules map only the observed account name; the user-risk rule maps the Entra account. No Windows SID is mislabeled as an Entra object ID
