...
- Expansion baseline: ten of thirty Sigma detections are implemented; Wave 1 is complete and the next bounded milestone must review Wave 2 separately rather than bulk-importing community rules
...
- Catalogue: ten experimental packages. `MSEC-DET-0001` through `MSEC-DET-0005` retain the original Windows service, Entra sign-in and Entra application-change coverage; Wave 1 adds `MSEC-DET-0006` Office-to-interpreter execution, `MSEC-DET-0007` encoded PowerShell, `MSEC-DET-0008` permanent Entra role assignment outside PIM, `MSEC-DET-0009` Entra security-information registration and `MSEC-DET-0010` active high-risk Entra user events
- Generated discovery catalogue: deterministic `catalog/index.json` and `CATALOGUE.md` are derived from the ten manifests, implementation-local fixture indexes, and explicit Sentinel preview profile; neither output contains timestamps, environment identifiers, or live result data
- Catalogue contract and validation: JSON Schema version 1, three generator tests, and a stale-output gate are included in the aggregate repository check; the current output reports ten implementations, thirty positive cases, forty negative cases, and nine Sentinel preview bindings
...
- Local `v0.4.0` candidate: the deterministic release build contains 148 members, including 147 allowlisted sources, all ten Sigma packages, seventy synthetic fixtures, nine Golden KQL queries and the read-only Sentinel inventory guide; its current SHA-256 is `02544984e3411febb60789b70c8e03278cb8e5ae5d3ff5f06c2fad10d9fc2c73`, while the published `v0.1.0` remains unchanged
- Portable implementations: ten structurally valid Sigma rules, one per package
- Synthetic evidence: thirty positive and forty negative flat event fixtures, all explicitly marked synthetic and all passing locally
...
- Sigma structural validation: exact-version gate, two-sided in-memory parser self-test, and automatic Package v1 `rule.yml` discovery validate ten sources containing ten rules
...
- Evaluator tests: six passing unit cases plus seventy passing committed fixture expectations
...
- Sentinel preview scope: nine rules are explicitly bound across `SigninLogs`, `AuditLogs`, `DeviceProcessEvents` and `AADUserRiskEvents`; all nine complete analyst-facing KQL queries match committed Golden snapshots
- Sentinel analytics-rule profile: version 1 JSON Schema and executable loader bind exactly the same nine detections to explicit schedule, threshold, suppression, event-grouping and incident settings; missing, additional, duplicated, reordered, active or malformed entries fail closed
- Sentinel analytics-rule renderer: all nine bindings produce deterministic Microsoft SecurityInsights API `2025-09-01` Scheduled-rule request bodies plus separate provenance manifests; logical metadata comes from `manifest.json`, KQL and entity mappings come from the reviewed compiler output contract, stable rule UUIDs derive from the immutable detection ID, and every rendered rule is disabled
...
- Generated coverage report: deterministic `coverage/index.json` and `COVERAGE.md` derive only from logical manifests, the Sentinel preview and the Sentinel data-source contract; the report records ten ATT&CK mappings across six techniques and four tactics, five logical data sources, four Sentinel source contracts and the one intentionally unbound detection without a percentage score or live state
...
- Live target probes: authorized read-only workspace checks accepted all nine generated predicates in bounded aggregate form. In Wave 1, `MSEC-DET-0006` and `MSEC-DET-0008` returned no match while `MSEC-DET-0007`, `MSEC-DET-0009` and `MSEC-DET-0010` returned a positive result; no raw row, aggregate count, user, device, tenant, subscription or workspace identifier is stored in the repository
...
- Current local validation: the complete aggregate repository check passes with 81 unit tests plus all structural, generated-output and Golden gates; all seventy synthetic fixture expectations pass, nine disabled Sentinel rule bodies render successfully and the deterministic `v0.4.0` release candidate builds successfully
