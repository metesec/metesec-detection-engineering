...
The read-only live probes for `MSEC-DET-0002` and `MSEC-DET-0003` used only aggregate counts in an existing user-authorized Microsoft Sentinel workspace. They established that `SigninLogs` and the required fields were queryable and that both compiled predicates were accepted. The legacy-client query produced a valid negative result; the high-risk sign-in query produced a valid positive result. No raw rows, user identifiers, tenant identifiers, workspace identifiers, or result counts are stored here. Neither result proves that a detection is production-ready.
